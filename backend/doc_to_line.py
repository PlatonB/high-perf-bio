...
def restore_info_cell(info_obj):
        ...
        ячейки VCF-INFO-столбца
        из MongoDB-объекта.
        Последний должен состоять
        из словаря бывших парных
        INFO-элементов и списка
        бывших одиночных INFO-флагов.
        То, как этот объект формируется,
        подробно описано в строке
        документации соответствующей
        функции программы создания БД.
        '''
        info_row = []
...
def restore_gt_cell(gt_obj):
        '''
        Функция восстановления
        ячейки VCF-GT-столбца
        из MongoDB-объекта.
        Последний должен представлять
        собой словарь, в котором ключи -
        бывшие FORMAT-элементы, а
        значения - бывшие GT-элементы.
        Историю формирования объекта
        можно понять по описанию
        соответствующей функции
        программы создания БД.
        '''
        gt_row = []
...
def restore_line(doc, trg_file_format, sec_delimiter):
        '''
        Функция восстановления полной
        строки из MongoDB-документа.
        То, как будет выглядеть строка,
        зависит от формата конечного файла.
        VCF- и BED-строки формируются
        в соответствии с официальными
        спецификациями этих форматов.
        Подробно тут это описывать мне лень,
        но могу дать разъяснения в Issues.
        Восстановление TSV осуществляется
        проще всего: вложенные списки
        схлопываются по заданному
        исследователем символу, а все
        элементы верхнего уровня - по
        табуляции, при этом не являющиеся
        списком элементы любой глубины
        приобретают строковый тип данных.
        '''
        del doc['_id']
