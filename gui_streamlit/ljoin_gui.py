        ...
        def __init__(self, ver):
                with st.form(key='ljoin'):
                        ...
                        self.lookup_field_path = st.text_input(label='lookup-field-path',
                                                               help='Точечный путь к полю, по которому пересекать или вычитать (применяется без by-loc; src-db-VCF: [[ID]]; src-db-BED: [[name]], src-db-TSV: [[первое после _id поле]])')
...
class AddWidgetsEn():
        ...
        def __init__(self, ver):
                with st.form(key='ljoin'):
                        ...
                        self.lookup_field_path = st.text_input(label='lookup-field-path',
                                                               help='Dot path to the field by which to intersect or subtract (applied without by-loc; src-db-VCF: [[ID]]; src-db-BED: [[name]], src-db-TSV: [[first field after _id]])')
                        self.action = st.radio(label='action', options=['intersect', 'subtract'],
                        ...
                        self.srt_order = st.radio(label='srt-order', options=['asc', 'desc'],
                                                  help='Order of sorting (applied with srt-field-group)')
                        self.proj_field_names = st.text_input(label='proj-field-names',
