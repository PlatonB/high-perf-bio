...
| --------- | -------------- |
| _MongoDB_ | собственно, размещает данные в базы и выполняет запросы |
| _PyMongo_ | позволяет работать с базами с помощью удобнейшего языка Python |
| _Streamlit_ | самый простой на свете (во всяком случае, в 2022 г.) конструктор веб-интерфейсов на Python |
...
- Скрипт-инсталлятор ставит _PyMongo_ через _Pip_, ибо _Conda_-версия этого драйвера неработоспособна.
