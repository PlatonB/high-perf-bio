...
По справке к _intersect_subtract_ вы уже знаете, что одна из коллекций, условно обозначенных как "левые", фильтруется по всем "правым". Левым пусть будет `c1`, а в качестве правых - `c2` и `c3`. Непосредственно обрабатываемое поле - `f1`. Общее представление об условях отбора документов `c1`:
...
То же самое в схематичном виде (0 - в правой коллекции нет совпадений, 1 - есть одно; a - alias (псевдоним)).
```
l, r1a: 0, r2a: 0
```
```
...
2. **Фильтрация**. Изучая предыдущий пункт, вы уже, наверное, догадались, что пересекать или вычитать — значит, фильтровать результаты объединения по количеству пустых или непустых результирующих списков.
