...
| intersect_subtract | фильтрует одни коллекции БД по наличию/отсутствию значений в других |
...
```
python3 intersect_subtract.py -D GTEx -T $HOME/Биоинформатика/результаты -l Whole_Blood.v8.egenes.txt -f rs_id_dbSNP151_GRCh38p7 -a subtract -d 0
```
...
| Действие | Глубина | Документ из `c1` сохраняется, если |
| -------- | ------- | ---------------------------- |
