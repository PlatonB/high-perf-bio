...
Разделяемые данные должны находиться в базе, созданной с помощью create_db.
...
''',
        ...
        opt_grp.add_argument('-f', '--chrom-field-name', metavar='[None]', dest='chrom_field_name', type=str,
                             help='Имя хромосомного поля (src-db-VCF, src-db-BED: не применяется, src-db-TSV: [[первое после _id поле]])')
...
of the MongoDB database by chromosomes.
...
For the program to process quickly, there needs index
of the field in which are the names of the chromosomes.
...
''',
        ...
        opt_grp.add_argument('-f', '--chrom-field-name', metavar='[None]', dest='chrom_field_name', type=str,
                             help='Name of the chromosomal field (src-db-VCF, src-db-BED: not applicable, src-db-TSV: [[first field after _id]])')
        opt_grp.add_argument('-k', '--proj-fields', metavar='[None]', dest='proj_fields', type=str,
        ...
        opt_grp.add_argument('-p', '--max-proc-quan', metavar='[4]', default=4, dest='max_proc_quan', type=int,
                             help='Maximum number of collections divided in parallel')
        args = arg_parser.parse_args()
        return args
